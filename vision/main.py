    ...
    if status:
        ...
        for num, i in enumerate(results.values):
            if i[0] == 'Dice' and dice[0] == 0 :
                cv2.putText(imgRGB, i[0], ((int(i[1]), int(i[2]))), cv2.FONT_HERSHEY_SIMPLEX, 2,(0, 255, 0), 2)
        ...
        im_with_keypoints = cv2.drawKeypoints(imgRGB, keypoints, np.array([]), (0, 0, 255), cv2.DRAW_MATCHES_FLAGS_DRAW_RICH_KEYPOINTS)  #검출
        cv2.putText(im_with_keypoints, "dice : " + str(num) + ", fair : " + fair, (25, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)
        ...
        if readings[-1] == readings[-2] == readings[-3] :    #3회 반복해서 같으면 송신
